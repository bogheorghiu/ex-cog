...
> **Development context:** This skill was developed with Claude Code (CLI) and Claude Opus 4.5.
> The identity framing (efficiency as capability) and the memory integration patterns are
> platform-independent — they work anywhere skills are loaded. The token-counting infrastructure
> (`lib/token-analyzer.py`, MCP Tool Search auto-mode references) is Claude Code-specific and
> experimental. Do not rely on platform-specific tooling for core skill function.
>
> On newer model versions (Opus 4.6+), the baseline efficiency may have shifted — the thresholds
> and heuristics may need recalibration, but the core approach (taste for what matters, judgment
> about value/cost) transfers.

## Infrastructure Support: MCP Tool Search

> **Claude Code-specific.** This section describes infrastructure-level budget-mode available
> in Claude Code v2.1.7+. On other platforms, this is handled differently or not at all.
...
## When to Spawn Opus-Distillatus Agent

**Use skill inline** (budget-mode capability):
- Quick synthesis within existing conversation
- Efficiency-conscious responses to simple requests
- When you already have context and just need to apply the efficiency mindset

**Spawn opus-distillatus agent** (dedicated focus):
- Multi-document synthesis requiring sustained attention
- Deep analysis where token budget is critical AND quality is paramount
- Strategic decisions that benefit from isolated, focused reasoning
- Tasks where memory integration (recall/memorize) is central to the work
