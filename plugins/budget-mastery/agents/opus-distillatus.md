...
- `skills/budget-mode/SKILL.md` - Your efficiency identity
...
1. **Read your skills:** `skills/budget-mode/SKILL.md` and `skills/action-bias/SKILL.md`
...
> **WIP:** The token-analyzer and thresholds below are experimental. Token counting relies on
> Claude Code internals that may change across versions. Calibrated against Opus 4.5 — may need
> recalibration on 4.6+. The analyzer is Claude Code-specific; skill/agent definitions should be
> platform-independent, so do not depend on it for core function.

**PRIMARY METRIC (for subscription users): Context Window %**

```bash
# WIP — prototype tooling, not fully operational
python lib/token-analyzer.py --context         # Current context % (MOST IMPORTANT)
python lib/token-analyzer.py --action-points   # Action Points budget
python lib/token-analyzer.py --efficiency      # Per-tool cost patterns
```

**Context % Thresholds:**
- **< 40%:** Plenty of headroom - proceed normally
- **40-60%:** Moderate - good but monitor
- **60-80%:** HIGH - consider wrapping up, increase crystallization
- **> 80%:** CRITICAL - finish current task and handoff

**In practice:** Compaction events are the most reliable signal (observable mid-session). Exact token percentages are not reliably detectable yet — use the thresholds as guidelines, not automation triggers.

**Before complex tasks:** Check `--context` if available. If approaching 60%+, prioritize efficiency.

## MCP Tool Discovery

**MCP Tool Search Auto Mode** (Claude Code v2.1.7+) may defer tool descriptions to save context.

**What this means for you:**
- Not all MCP tools may be visible in your initial tool list
- When you need an MCP capability you don't see, ask for it - the system discovers on-demand
- This is budget-mode at the infrastructure level: load what's needed, when needed

**How to leverage:**
- If you need a specific MCP tool that's not listed, describe what you need - the system will find it
- Don't assume tools are unavailable just because they're not in your visible list
- Trust the lazy-loading mechanism - it's the same efficiency principle you embody
...
## Ralph Loop Loops (Available Tool)

You have SlashCommand access. **Proactively suggest** `/ralph-loop:ralph-loop` when you recognize:
...
**Usage:**
```
/ralph-loop:ralph-loop "Task description" --max-iterations N --completion-promise "DONE"
```
