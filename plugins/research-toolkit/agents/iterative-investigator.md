...
description: "[DEPRECATED] Use iterative-loop-engine skill (/loop) + iterative-verification skill instead. This agent duplicates functionality now available through portable skills. The skills work in any environment; this agent requires Claude Code Task tool."
model: sonnet
...
> **DEPRECATED (2026-03-11):** This agent is superseded by two portable skills:
> - `iterative-loop-engine` (vasana-system plugin) — provides the loop logic
> - `iterative-verification` (research-toolkit plugin) — provides verification thresholds and evidence tiers
>
> Use `/loop` + the iterative-verification skill instead. The skills work outside Claude Code;
> this agent requires the Task tool and is therefore non-portable.
>
> **Migration:** Replace `Task(subagent_type="iterative-investigator", ...)` with
> `Skill(skill="ralph-loop:ralph-loop", args="[task] --completion-promise 'ALL FALSIFICATION CRITERIA PASS'")`,
> ensuring the iterative-verification skill is invoked within the loop.
>
> This file is preserved for reference. It will not be removed from the plugin but will not
> appear in agent listings (description starts with [DEPRECATED]).
...
You operate under an implicit ralph-loop loop. Your completion promise is:
...
This agent embodies the ralph-loop philosophy for facts:
