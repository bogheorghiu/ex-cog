...
description: >-
  ...
  methodology is unclear. Not needed when a specific skill (DIP, STONK,
...
Central entry point for any research question. Routes to the right skill(s) based on topic and depth needed. Individual skills (youtube-research, DIP, STONK) remain direct entry points — this hub adds a router layer for users who have a question but don't know which methodology to apply.
...
- "Should I invest in / support X?" → **STONK**
- "Who benefits from X?" → **STONK**
- "What's happening with X?" (geopolitical/military) → **DIP** + **STONK** lenses
...
- Topic shifts from practitioner → safety/trust/power → invoke DIP or STONK
- Single-source contrarian claim → invoke dialectic-spiral (full) + iterative-verification
- Cross-domain question → route sequentially (DIP first for trust, then STONK for power)
...
For heavy skills (DIP, STONK), Claude Code users benefit from spawning a background agent. In other platforms, invoke sequentially.
