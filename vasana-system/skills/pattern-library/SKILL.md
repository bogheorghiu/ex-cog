...
**Note**: To discover NEW patterns, see `record-pattern` skill. For stuckness detection, see `break-pattern` skill. The catalog above is hand-maintained — adding a new pattern requires appending an entry to this list. Automating this is tracked upstream as `bogheorghiu/Claude-Code-Projects#403`.
